...
class TestSauceDemo:
    ...
    def test_complete_purchase(self, firefox_driver):
        ...
        with allure.step("3. Проверить количество товаров в корзине"):
            ...
            with allure.step(
                f"Проверить: ожидается {expected_count}, получено {cart_count}"
            ):
                error_msg = (
                    f"Ожидалось {expected_count} товара в корзине, "
                    f"получено {cart_count}"
                )
                assert cart_count == expected_count, error_msg
        ...
        with allure.step("5. Проверить содержимое корзины"):
            ...
            with allure.step("Проверить наличие всех добавленных товаров"):
                for product in products_to_add:
                    found = any(product in item for item in cart_items)
                    assert found, f"Товар {product} не найден в корзине"

            items_list = "Товары в корзине:\n" + "\n".join(cart_items)
            allure.attach(
                items_list,
        ...
        with allure.step("8. Проверить итоговую стоимость"):
            ...
            with allure.step(
                f"Сравнить: ожидается ${expected_total}, "
                ...
                assert total_price == expected_total, error_msg
        ...
        with allure.step("9. Завершить покупку"):
            ...
            success_element = driver.find_element("class name",
                                                  "complete-header")
            success_text = success_element.text
            expected_text = "Thank you for your order"
            assert expected_text in success_text, "Заказ не завершен успешно"
    ...
    @allure.description(
        "Проверка добавления одного товара и обновления счетчика корзины"
    )
    ...
    def test_add_single_product_to_cart(self, firefox_driver, product_name):
        ...
        """
        with allure.step(
            "Тестирование добавления товара: {}".format(product_name)
        ):
            driver = firefox_driver
            login_page = LoginPage(driver)
            products_page = ProductsPage(driver)
            ...
            with allure.step("Проверить обновление счетчика корзины"):
                ...
                assert new_count == initial_count + 1, error_msg
